...
def reconstruct_from_binary_patterns():
    ...
    for i in range(0,15):
        ...
        on_mask_img = np.zeros((h,w), dtype=np.float32)
        tempv = np.zeros((h,w), dtype=np.float32)

        # this code corresponds with the binary pattern code
        bit_code = np.uint16(1 << i)
        
        #scan_bits = bit_code | (int)(on_mask)
    ...
    camera_points = []
    projector_points = []
    half_img = np.zeros((h,w,3), np.float32)
    s=0
    for x in range(w):
        for y in range(h):
            ...
            #projector_points[s][0] = p_x
            #projector_points[s][1] = p_y
            #s = s+1
